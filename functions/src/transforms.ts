...
export function transformFixtureDetails(
  ...
  const mapPlayers = (players: Array<{ player: { id: number; name: string; number: number; pos: string } }>) =>
    players.map((p) => ({
      id: p.player.id,
  ...
  // Transform stats
  let matchStats: Record<string, any> | null = null;
  if (homeStatsData && awayStatsData) {
    const getStat = (teamStats: ApiFixtureStats, type: string): number => {
